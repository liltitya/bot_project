import telebot
from TOKEN import token_qiwi as token,admins
import requests
import os
...
def delete_user(chatID):
    ...
    if str(chatID)+'\n' in just_users:
        ...
        with open('new_users_qiwi.txt', 'w') as new_users:
            for i in range(len(just_users)):
                new_users.write(just_users[i])
        os.remove('users_qiwi.txt')
        os.rename('new_users_qiwi.txt', 'users_qiwi.txt')
        return 1
...
def start_message(message):
    if add_user(message.chat.id,message.chat.username):
        params = {
            'chat_id': message.chat.id,
            'text': f'Привет! Здесь будут появляться выгодные для тебя предложения! В этом боте первостепенным является Qiwi Wallet. Удачных сделок!'
        }
