import telebot
from TOKEN import token_vip as token,admins
import requests
import os
...
def delete_user(chatID):
    ...
    if str(chatID)+'\n' in just_users:
        ...
        with open('new_users_vip.txt', 'w') as new_users:
            for i in range(len(just_users)):
                new_users.write(just_users[i])
        os.remove('users_vip.txt')
        os.rename('new_users_vip.txt', 'users_vip.txt')
        return 1
...
def start_message(message):
    if add_user(message.chat.id,message.chat.username):
        params = {
            'chat_id': message.chat.id,
            'text': 'Привет! Здесь будут появляться выгодные предложения  >1% ! В этом боте первостепенным являются три банка : Сбербанк, Тинькофф, Райффайзен. Удачных сделок!'
        }
